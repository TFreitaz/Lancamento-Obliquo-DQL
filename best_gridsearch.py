...
# to_search = config_list(param_grid, 1)
...
for i, conf in enumerate(to_search):
    ...
    env_settings = {
        "theta_disc": get_config("theta_disc", 10, int, conf),  # número de pontos de discretização do ângulo de lancamento
        "vel_disc": get_config("vel_disc", 47, int, conf),  # número de pontos de dicretização da velocidade de lancamento
    ...
    n_episodes = get_config("n_episodes", 19627, int, conf)  # número de episodios a serem executados
    decay = get_config("decay", 0.0378505, float, conf)  # decaimento da taxa de aleatoriedade

    agent_settings = {
        "num_states": 3,  # número de parâmetros de um estado = (distancia)
        "gamma": 0,  # incremento por ações futuras
        "min_experiences": get_config("min_experiences", 716, int, conf),  # mínimo de experiências aleatórias
        "max_experiences": get_config("max_experiences", 6715, int, conf),  # máximo de experiências aleatórias
    ...
    min_eps = get_config("min_eps", 0.126184, float, conf)  # mínima taxa de aleatoriedade
    max_eps = get_config("max_eps", 0.53168, float, conf)  # máxima taxa de aleatoriedade
